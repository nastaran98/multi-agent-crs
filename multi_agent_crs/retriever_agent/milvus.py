...
def create_vector_store(config):
    ...
    vectorstore = Milvus.from_documents(docs, openai_ef, collection_name="milvus_db")
    return vectorstore
...
def load_vectore_store(config):
    ...
    if not os.path.exists(db_path):
        ...
        vector_store = Milvus(persist_directory="milvus_db", embedding_function=openai_ef)
    return vector_store
...
def create_retriever(config):
    ...
    allowed_comparators = [
        ...
        "$in",  # Less than or equal to (number)
    ]
    ...
    examples = [
        ...
        (
            ...
            {
                "query": "",
                "filter": 'or(in("genre", ["کارآگاهی"]), in("short_description", ["کارآگاهی"]))',
            },
        ),
        (
            ...
            {
                "query": "",
                "filter": 'or(in("genre", ["اقتصاد"]), in("short_description", ["اقتصاد"]))',
            },
        )
    ]
    ...
    return retriever, query_constructor

def retriever_tool_(config):
    retriever, query_constructor = create_retriever(config)
